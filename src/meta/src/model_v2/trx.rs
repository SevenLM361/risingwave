...
mod tests {
    ...
    mod test_transactional {
        use sea_orm::entity::prelude::*;
        #[derive(Clone, Debug, PartialEq, DeriveEntityModel, Eq)]
        #[sea_orm(table_name = "test_transactional")]
        pub struct Model {
            #[sea_orm(primary_key)]
            pub key: String,
            pub value: String,
    ...
    impl Transactional for TestTransactional {
        async fn upsert_in_transaction(&self, trx: &Transaction) -> MetadataModelResult<()> {
            let m = test_transactional::ActiveModel {
                key: sea_orm::Set(self.key.to_owned()),
                value: sea_orm::Set(self.key.to_owned()),
            };
            m.save(trx).await.unwrap();
            Ok(())
        }

        async fn delete_in_transaction(&self, trx: &Transaction) -> MetadataModelResult<()> {
            test_transactional::Entity::delete_by_id(self.key.to_owned())
                .exec(trx)
                .await
                .unwrap();
            Ok(())
